...
def my_load_pretrained(config, model, logger):
    logger.info(f"==============> Load Pre-train from {config.MODEL.PRETRAINED} ....................")
    logger.info(f"==============> Creating Model type is {config.MODEL.TYPE} ....................")
    try:
        checkpoint = torch.load(config.MODEL.PRETRAINED, map_location='cpu')
        state_dict = checkpoint['model']
    except:
        checkpoint = torch.jit.load(config.MODEL.PRETRAINED, map_location="cpu").visual
