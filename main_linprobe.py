...
from utils import load_checkpoint, load_pretrained, save_checkpoint, NativeScalerWithGradNormCount, auto_resume_helper, \
    reduce_tensor, my_load_pretrained
...
def build_model_from_timm(config):
    ...
    model.emned = torch.nn.Linear(1536, 64, bias=False)
    model.arc_head = Arcface(64, config.MODEL.NUM_CLASSES)
    return model
...
def main(config):
    ...
    #logger.info(f"Creating model from timm:{config.MODEL.TYPE}/{config.MODEL.NAME}")
    ...
    for _, p in model.embed.named_parameters():
    ...
    if config.MODEL.PRETRAINED and (not config.MODEL.RESUME):
        my_load_pretrained(config, model_without_ddp, logger)
        #acc1, acc5, loss = validate(config, data_loader_val, model)
        #logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
