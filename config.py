...
_C.DATA.DATA_PATH = ''
_C.DATA.SPLIT = 'train_list_all_v1.txt'
# Dataset name
_C.DATA.DATASET = 'universal'
# Input image size
...
# ViT-L(CLIP) Transformer parameters
_C.MODEL.VIT = CN()
_C.MODEL.VIT.PATCH_SIZE = 14
_C.MODEL.VIT.IN_CHANS = 3
_C.MODEL.VIT.EMBED_DIM = 768
_C.MODEL.VIT.DEPTHS = 24
_C.MODEL.VIT.NUM_HEADS = 16
_C.MODEL.VIT.WIDTH = 1024
_C.MODEL.VIT.MLP_RATIO = 4.
_C.MODEL.VIT.QKV_BIAS = True
_C.MODEL.VIT.QK_SCALE = None
_C.MODEL.VIT.APE = False
...
def get_config_infer(args):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config_infer(config, args)

    return config


def update_config_infer(config, args):
    _update_config_from_file(config, args.cfg)
