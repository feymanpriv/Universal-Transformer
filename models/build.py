...
from .vision_transformer import VisualTransformer
from .swin_transformer_infer import SwinTransformer_infer
...
def build_model(config):
    ...
    if model_type == 'swin':
    ...
    elif model_type == 'swin_infer':
        model = SwinTransformer_infer(img_size=config.DATA.IMG_SIZE,
                                    patch_size=config.MODEL.SWIN.PATCH_SIZE,
                                    in_chans=config.MODEL.SWIN.IN_CHANS,
                                    num_classes=config.MODEL.NUM_CLASSES,
                                    embed_dim=config.MODEL.SWIN.EMBED_DIM,
        ...
        model = VisualTransformer(num_classes=config.MODEL.NUM_CLASSES,
                                  input_resolution=config.DATA.IMG_SIZE,
                                  patch_size=config.MODEL.VIT.PATCH_SIZE,
                                  width=config.MODEL.VIT.WIDTH,
                                  layers=config.MODEL.VIT.DEPTHS,
                                  heads=config.MODEL.VIT.NUM_HEADS,
                                  output_dim=config.MODEL.VIT.EMBED_DIM)
